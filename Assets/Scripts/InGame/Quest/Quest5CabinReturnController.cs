...
/// <summary>
/// QUEST 5: Handles player's return to cabin for FINAL Java Syntax Recap
/// - Objective 1: Player returns to cabin (trigger zone)
/// - Objective 2: Arin gives final comprehensive Java syntax recap dialogue
/// - After dialogue: Objective 3 activates (Complete Syntax Trial) - Arin remains idle at cabin
/// </summary>
[RequireComponent(typeof(Collider2D))]
public class Quest5CabinReturnController : MonoBehaviour
{
    [Header("Quest 5 Settings")]
    [Tooltip("Quest ID for Quest 5")]
    [SerializeField] private string questId = "Q5_SyntaxTrial";
    
    [Tooltip("Objective 1: Return to cabin")]
    [SerializeField] private string returnToCabinObjective = "Return to Arin's Cabin";
    
    [Tooltip("Objective 2: Listen to final recap")]
    [SerializeField] private string recapObjective = "Listen to Arin's Final Recap";
    
    [Tooltip("Dialogue conversation ID for Arin's final Java syntax recap")]
    [SerializeField] private string recapDialogueId = "Arin_05_SyntaxRecap";
    ...
    [Header("Timing")]
    [Tooltip("Delay before starting recap dialogue after player enters cabin")]
    [SerializeField] private float recapDialogueDelay = 0.5f;
    ...
    void Awake()
    {
        ...
        {
            col.isTrigger = true;
            Debug.LogWarning($"Quest5CabinReturnController on {gameObject.name}: Collider was not set as trigger. Fixed automatically.");
        }
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            Debug.LogError("[Q5Cabin] QuestManager not found in scene!");
            return;
        }
        ...
        if (currentQuest == null || currentQuest.questId != questId)
        {
            if (debugMode)
            {
                Debug.Log($"[Q5Cabin] Quest 5 not active yet. Current quest: {currentQuest?.questId ?? "None"}");
            }
            return;
        }
        ...
        if (debugMode)
        {
            Debug.Log($"[Q5Cabin] Player returned to cabin! Starting Quest 5 FINAL RECAP sequence...");
        }
    ...
    private IEnumerator HandleCabinReturnSequence()
    {
        ...
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 1: Completing 'Return to cabin' objective...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 2: Caching components...");
        }
        ...
        if (arinAI == null)
        {
            Debug.LogError("[Q5Cabin] Arin NPC not found! Cannot start final recap.");
            yield break;
        }
        ...
        if (player == null)
        {
            Debug.LogError("[Q5Cabin] Player not found!");
            yield break;
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 3: Hiding UI...");
        }
        
        HideUIImmediately();
        yield return new WaitForEndOfFrame();
        
        // ============= PHASE 4: PREPARE ARIN FOR DIALOGUE =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 4: Preparing Arin for final recap...");
        }
        ...
        // ============= PHASE 5: INTELLIGENT DISTANCE ADJUSTMENT =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 5: Adjusting Arin's position for conversation...");
        }
        
        yield return StartCoroutine(AdjustArinDistanceIntelligently(player));
        
        // ============= PHASE 6: ENSURE COMPLETE STOP =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 6: Ensuring Arin is completely stopped...");
        }
        ...
        // ============= PHASE 7: FACE EACH OTHER =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 7: Characters facing each other...");
        }
        ...
        // ============= PHASE 8: VERIFY FACING =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 8: Verifying character facing...");
        }
        
        VerifyAndCorrectFacing(player);
        
        // ============= PHASE 9: WAIT BEFORE DIALOGUE =============
        yield return new WaitForSeconds(recapDialogueDelay);
        
        // ============= PHASE 10: START FINAL RECAP DIALOGUE =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 10: Starting FINAL JAVA SYNTAX RECAP dialogue...");
        }
        ...
        {
            DialogueManager.Instance.StartConversation(recapDialogueId);
            DialogueManager.Instance.OnConversationCompleted += OnRecapCompleted;
        }
        else
        {
            Debug.LogWarning("[Q5Cabin] DialogueManager not found. Simulating recap completion.");
            yield return new WaitForSeconds(2f);
            OnRecapCompleted(recapDialogueId);
        }
    }

    #region Intelligent Distance Adjustment (Same as Quest 3/4)
    ...
    private IEnumerator AdjustArinDistanceIntelligently(Transform player)
    {
        ...
        Debug.Log($"[Q5Cabin] Initial distance check:");
        ...
        {
            Debug.Log("[Q5Cabin] ✓ Distance is perfect - no adjustment needed");
            yield break;
        }

        if (currentDistance <= overlappingThreshold)
        {
            Debug.LogWarning($"[Q5Cabin] ⚠ OVERLAPPING DETECTED! Distance: {currentDistance:F2} units");
        ...
        Debug.Log($"[Q5Cabin] ✓ Final distance: {finalDistance:F2} units (target: {conversationDistance:F2})");
    }

    private IEnumerator ForceSeparation(Transform player)
    {
        Debug.Log("[Q5Cabin] FORCED SEPARATION SEQUENCE INITIATED");
        ...
        Debug.Log($"[Q5Cabin] Forced separation target: {targetPosition}");
        ...
        while (iterationCount < maxIterations)
        {
            ...
            {
                Debug.Log($"[Q5Cabin] ✓ Forced separation complete after {iterationCount} iterations");
                break;
            }
            ...
            // Update sprite facing during movement
    ...
    private float DetermineIntelligentSeparationDirection(Vector3 playerPos, Vector3 arinPos)
    {
        if (playerMovement != null)
        {
            ...
            if (Mathf.Abs(playerFacing) > 0.01f)
            {
                ...
                Debug.Log($"[Q5Cabin] Separation: Behind player (direction={behindPlayer})");
                return behindPlayer;
            }
        }

        Debug.Log($"[Q5Cabin] Separation: Using default direction ({defaultSeparationDirection})");
    ...
    {
        Debug.Log($"[Q5Cabin] Starting continuous movement: {moveDirection}");
        ...
        while (iterationCount < maxIterations)
        {
            ...
            {
                Debug.Log($"[Q5Cabin] ✓ ARRIVED at target distance after {iterationCount} iterations!");
                break;
            }
            ...
            {
                Debug.LogWarning("[Q5Cabin] Overlapping detected - triggering forced separation");
                yield return StartCoroutine(ForceSeparation(player));
                yield break;
            }
    ...
    private void FaceBothCharactersTogether(Transform player)
    {
        ...
        {
            Debug.LogError("[Q5Cabin] Cannot face characters - missing references!");
            return;
        }
        ...
        Debug.Log($"[Q5Cabin] Facing calculation:");
        ...
        Debug.Log("[Q5Cabin] ✓ Both characters oriented to face each other");
    }
    ...
    private void VerifyAndCorrectFacing(Transform player)
    {
        ...
        if (arinSprite != null)
        {
            ...
            {
                Debug.LogWarning($"[Q5Cabin] ⚠ Correcting Arin's facing!");
        ...
        if (playerMovement != null)
        {
            ...
            if (playerSprite != null)
            {
                ...
                {
                    Debug.LogWarning($"[Q5Cabin] ⚠ Correcting Player's facing!");
        ...
        if (needsCorrection)
        {
            Debug.Log("[Q5Cabin] ✓ Facing corrected!");
        }
        else
        {
            Debug.Log("[Q5Cabin] ✓ Facing verification passed");
        }
    }
    
    #endregion

    #region Dialogue Completion Handler
    
    private void OnRecapCompleted(string conversationId)
    {
        if (conversationId != recapDialogueId) return;
        
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] ✓ Final recap dialogue completed!");
        }
        
        if (DialogueManager.Instance != null)
            DialogueManager.Instance.OnConversationCompleted -= OnRecapCompleted;
        
        StartCoroutine(HandlePostRecapSequence());
    }

    private IEnumerator HandlePostRecapSequence()
    {
        // ============= PHASE 11: COMPLETE OBJECTIVE 2 (FINAL RECAP) =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 11: Completing 'Listen to final recap' objective...");
        }
        
        QuestManager.Instance.CompleteObjectiveByTitle(recapObjective);
        ...
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 12: Restoring UI...");
        }
        
        yield return StartCoroutine(RestoreUIGradually());
        
        // ============= PHASE 13: KEEP ARIN STATIONARY =============
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] PHASE 13: Setting Arin to idle state at cabin...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] ✓✓✓ Quest 5 final recap completed! Arin waiting at cabin, Objective 3 (Syntax Trial) now active.");
        }
    }
    ...
    private void FinalizeArinState()
    {
        ...
        Debug.Log($"[Q5Cabin] Arin is now in idle state at cabin position: {arinAI.transform.position}");
        Debug.Log("[Q5Cabin] Arin AI disabled - will remain at cabin for Syntax Trial");
    }
    ...
    private IEnumerator RestoreUIGradually()
    {
        ...
        if (debugMode)
        {
            Debug.Log("[Q5Cabin] UI fully restored");
        }
    }
    ...
    void OnDrawGizmos()
    {
        ...
        UnityEditor.Handles.Label(transform.position + Vector3.up * 0.5f, 
            $"QUEST 5: Cabin Return Trigger (FINAL RECAP)\n{returnToCabinObjective}\n{recapObjective}", 
