...
/// <summary>
/// QUEST 4: Handles player's return to cabin for Strings and Printing lecture
...
/// - After dialogue: Objective 3 activates (Complete String Exercises) - Arin remains idle at cabin
/// </summary>
[RequireComponent(typeof(Collider2D))]
public class Quest4CabinReturnController : MonoBehaviour
{
    [Header("Quest 4 Settings")]
    [Tooltip("Quest ID for Quest 4")]
    [SerializeField] private string questId = "Q4_StringsPrinting";
    ...
    [SerializeField] private string lectureObjective = "Listen to Arin's Lesson: Strings and Printing";
    
    [Tooltip("Dialogue conversation ID for Arin's lecture")]
    [SerializeField] private string lectureDialogueId = "Arin_04_StringsLecture";
    ...
    [Header("Timing")]
    [Tooltip("Delay before starting lecture dialogue after player enters cabin")]
    [SerializeField] private float lectureDialogueDelay = 0.5f;
    ...
    private Vector3 arinOriginalPosition;
    ...
    void Awake()
    {
        ...
        {
            col.isTrigger = true;
            Debug.LogWarning($"Quest4CabinReturnController on {gameObject.name}: Collider was not set as trigger. Fixed automatically.");
        }
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            Debug.LogError("[Q4Cabin] QuestManager not found in scene!");
            return;
        }
        ...
        if (currentQuest == null || currentQuest.questId != questId)
        {
            if (debugMode)
            {
                Debug.Log($"[Q4Cabin] Quest 4 not active yet. Current quest: {currentQuest?.questId ?? "None"}");
            }
            return;
        }
        ...
        if (debugMode)
        {
            Debug.Log($"[Q4Cabin] Player returned to cabin! Starting Quest 4 dialogue sequence...");
        }
    ...
    private IEnumerator HandleCabinReturnSequence()
    {
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 1: Completing 'Return to cabin' objective...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 2: Caching components...");
        }
        ...
        if (arinAI == null)
        {
            Debug.LogError("[Q4Cabin] Arin NPC not found! Cannot start dialogue.");
            yield break;
        }
        ...
        if (player == null)
        {
            Debug.LogError("[Q4Cabin] Player not found!");
            yield break;
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 3: Storing Arin's original position...");
        }
        
        arinOriginalPosition = arinAI.transform.position;
        Debug.Log($"[Q4Cabin] Arin's original position: {arinOriginalPosition}");
        
        // ============= PHASE 4: HIDE UI =============
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 4: Hiding UI...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 5: Preparing Arin for dialogue...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 6: Adjusting Arin's position for conversation...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 7: Ensuring Arin is completely stopped...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 8: Characters facing each other...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 9: Verifying character facing...");
        }
        
        VerifyAndCorrectFacing(player);
        
        // ============= PHASE 10: WAIT BEFORE DIALOGUE =============
        yield return new WaitForSeconds(lectureDialogueDelay);
        
        // ============= PHASE 11: START LECTURE DIALOGUE =============
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 11: Starting lecture dialogue...");
        }
        ...
        {
            Debug.LogWarning("[Q4Cabin] DialogueManager not found. Simulating lecture completion.");
            yield return new WaitForSeconds(2f);
            OnLectureCompleted(lectureDialogueId);
        }
    }

    #region Intelligent Distance Adjustment (Same as Quest 3)
    ...
    private IEnumerator AdjustArinDistanceIntelligently(Transform player)
    {
        ...
        Debug.Log($"[Q4Cabin] Initial distance check:");
        ...
        {
            Debug.Log("[Q4Cabin] ✓ Distance is perfect - no adjustment needed");
            yield break;
        }

        if (currentDistance <= overlappingThreshold)
        {
            Debug.LogWarning($"[Q4Cabin] ⚠ OVERLAPPING DETECTED! Distance: {currentDistance:F2} units");
        ...
        Debug.Log($"[Q4Cabin] ✓ Final distance: {finalDistance:F2} units (target: {conversationDistance:F2})");
    }

    private IEnumerator ForceSeparation(Transform player)
    {
        Debug.Log("[Q4Cabin] FORCED SEPARATION SEQUENCE INITIATED");
        ...
        Debug.Log($"[Q4Cabin] Forced separation target: {targetPosition}");
        ...
        while (iterationCount < maxIterations)
        {
            ...
            {
                Debug.Log($"[Q4Cabin] ✓ Forced separation complete after {iterationCount} iterations");
                break;
            }
    ...
    private float DetermineIntelligentSeparationDirection(Vector3 playerPos, Vector3 arinPos)
    {
        if (playerMovement != null)
        {
            ...
            if (Mathf.Abs(playerFacing) > 0.01f)
            {
                ...
                Debug.Log($"[Q4Cabin] Separation: Behind player (direction={behindPlayer})");
                return behindPlayer;
            }
        }

        Debug.Log($"[Q4Cabin] Separation: Using default direction ({defaultSeparationDirection})");
    ...
    {
        Debug.Log($"[Q4Cabin] Starting continuous movement: {moveDirection}");
        ...
        while (iterationCount < maxIterations)
        {
            ...
            {
                Debug.Log($"[Q4Cabin] ✓ ARRIVED at target distance after {iterationCount} iterations!");
                break;
            }
            ...
            {
                Debug.LogWarning("[Q4Cabin] Overlapping detected - triggering forced separation");
                yield return StartCoroutine(ForceSeparation(player));
                yield break;
            }
    ...
    private void FaceBothCharactersTogether(Transform player)
    {
        ...
        {
            Debug.LogError("[Q4Cabin] Cannot face characters - missing references!");
            return;
        }
        ...
        Debug.Log($"[Q4Cabin] Facing calculation:");
        ...
        Debug.Log("[Q4Cabin] ✓ Both characters oriented to face each other");
    }
    ...
    private void VerifyAndCorrectFacing(Transform player)
    {
        ...
        if (arinSprite != null)
        {
            ...
            {
                Debug.LogWarning($"[Q4Cabin] ⚠ Correcting Arin's facing!");
        ...
        if (playerMovement != null)
        {
            ...
            if (playerSprite != null)
            {
                ...
                {
                    Debug.LogWarning($"[Q4Cabin] ⚠ Correcting Player's facing!");
        ...
        if (needsCorrection)
        {
            Debug.Log("[Q4Cabin] ✓ Facing corrected!");
        }
        else
        {
            Debug.Log("[Q4Cabin] ✓ Facing verification passed");
        }
    }
    ...
    private void OnLectureCompleted(string conversationId)
    {
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] ✓ Lecture dialogue completed!");
        }
    ...
    private IEnumerator HandlePostLectureSequence()
    {
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 12: Completing 'Listen to lecture' objective...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] PHASE 13: Restoring UI...");
        }
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] ✓✓✓ Quest 4 dialogue completed! Arin waiting at cabin, Objective 3 (String Exercises) now active.");
        }
    }
    ...
    private IEnumerator RestoreUIGradually()
    {
        ...
        if (debugMode)
        {
            Debug.Log("[Q4Cabin] UI fully restored");
        }
    }
