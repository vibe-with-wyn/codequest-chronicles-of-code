    ...
    public class PlayerAttackData
    {
        ...
        public float FireballSpawnDelay => fireballSpawnDelay;
    }
    ...
    public class PlayerMovement : MonoBehaviour
    {
        ...
        [Header("Sound Effects - Player States")]
        ...
        [SerializeField] private AudioClip attack5Sound;

        [Header("Sound Settings")]
        [SerializeField] private float soundLoopInterval = 0.5f;
        ...
        // Sound effect state tracking - LOOPING SOUNDS ONLY
        private float soundLoopTimer = 0f;
        private AudioClip lastLoopingSound = null;
        private bool wasJumpingSoundPlayed = false;
        private bool wasFallingSoundPlayed = false;
        private bool wasLandingSoundPlayed = false;
        ...
        private void InitializeComponents()
        {
            ...
            if (audioSource == null)
            {
                ...
                if (audioSource == null)
                {
                    ...
                    Debug.Log("AudioSource component added to Player and configured");
                }
                else
                {
                    Debug.Log("AudioSource found on Player");
                }
            }
        ...
        private void ConfigureAudioSource()
        {
            ...
            Debug.Log("AudioSource configured with optimal settings");
        }
        ...
        private void ResetAllStates()
        {
            ...
            soundLoopTimer = 0f;
            wasJumpingSoundPlayed = false;
            wasFallingSoundPlayed = false;
            wasLandingSoundPlayed = false;
        ...
        private void HandleJump()
        {
            ...
            if (uiController != null && uiController.IsJumping && isGrounded && !isHurt && !isJumping)
            {
                ...
                isJumping = true;
                wasJumpingSoundPlayed = true;
                wasLandingSoundPlayed = false;

                SetAnimatorBool("isJumping", true);
            
                // CRITICAL FIX: Ensure looping sounds are completely stopped before jump
                StopLoopingSounds();
            
                // Play jump sound
                PlayOneTimeSoundEffect(jumpSound);
        ...
        private void HandleAnimations()
        {
            ...
            // Handle falling sound - play once when falling starts
            if (rb.linearVelocity.y < 0 && !isGrounded)
            {
                SetAnimatorBool("isFalling", true);
                if (!wasFallingSoundPlayed)
                {
                    PlayOneTimeSoundEffect(fallSound);
                    wasFallingSoundPlayed = true;
                }
            }
            else
            {
            ...
            // Note: Landing sound now handled in OnCollisionEnter2D for better sync
        }
        ...
        private void UpdateLoopingSoundEffects()
        {
            ...
            // Determine current state - ONLY for looping sounds (idle, walk, run)
            bool isCurrentlyIdle = !isRunning && isGrounded && !isAttacking && !isIntroWalking && !isJumping && !isHurt;
            bool isCurrentlyRunning = isRunning && !isAttacking && isGrounded;
            bool isCurrentlyWalking = isIntroWalking;

            // Stop looping sound if state changed
            if (lastLoopingSound != null)
            {
                if ((lastLoopingSound == idleSound && !isCurrentlyIdle) ||
                    (lastLoopingSound == runSound && !isCurrentlyRunning) ||
                    (lastLoopingSound == walkSound && !isCurrentlyWalking))
                {
                    audioSource.Stop();
                    lastLoopingSound = null;
                    soundLoopTimer = 0f;
                    Debug.Log("Stopped looping sound due to state change");
                    return;
                }
            }

            // Play appropriate looping sound for current state
            if (isCurrentlyWalking)
            {
                // Walking during intro - loop walk sound
                if (soundLoopTimer <= 0 && walkSound != null && lastLoopingSound != walkSound)
                {
                    PlayLoopingSound(walkSound);
                    Debug.Log("Started looping walk sound");
                }
            }
            else if (isCurrentlyRunning)
            {
                // Running - loop run sound
                if (soundLoopTimer <= 0 && runSound != null && lastLoopingSound != runSound)
                {
                    PlayLoopingSound(runSound);
                    Debug.Log("Started looping run sound");
                }
            }
            else if (isCurrentlyIdle)
            {
                // Idle - loop idle sound
                if (soundLoopTimer <= 0 && idleSound != null && lastLoopingSound != idleSound)
                {
                    PlayLoopingSound(idleSound);
                    Debug.Log("Started looping idle sound");
                }
            }
            else
            {
                // No valid looping state - ensure sound is stopped
                if (audioSource.isPlaying && lastLoopingSound != null)
                {
                    audioSource.Stop();
                    Debug.Log("Stopped looping sound - player in transition state");
                }
                lastLoopingSound = null;
                soundLoopTimer = 0f;
            }
        ...
        public void TriggerAttack(int attackIndex)
        {
            ...
            {
                // Stop any looping sound when attack starts
                StopLoopingSounds();
                ...
                // Play attack sound effect ONCE - NOT looping
        ...
        private void ResetAttackState()
        {
            ...
            // Reset sound loop timer to resume looping sounds after attack
            soundLoopTimer = 0f;
        ...
        /// <summary>
        /// Plays a one-time sound effect (jump, fall, land, attack, hurt, die)
        /// </summary>
        private void PlayOneTimeSoundEffect(AudioClip clip)
        {
            ...
            Debug.Log($"Playing one-time sound: {clip.name}");
        }

        /// <summary>
        /// Plays a looping sound effect (idle, walk, run)
        /// </summary>
        private void PlayLoopingSound(AudioClip clip)
        {
            ...
            soundLoopTimer = soundLoopInterval;
            Debug.Log($"Playing looping sound: {clip.name}");
        }
        ...
        {
            if (lastLoopingSound != null && audioSource.isPlaying)
            {
                audioSource.Stop();
                lastLoopingSound = null;
                soundLoopTimer = 0f;
                Debug.Log("Stopped looping sounds");
            }
        }
        #endregion
        ...
        void OnCollisionEnter2D(Collision2D collision)
        {
            ...
            {
                bool wasInAir = !isGrounded; // Store previous airborne state
                ...
                // Play landing sound only if player was in the air and hasn't played it yet
                if (wasInAir && !wasLandingSoundPlayed)
                {
                    PlayOneTimeSoundEffect(landSound);
                    wasLandingSoundPlayed = true;
                    Debug.Log("Landing sound played on ground collision");
                }
        ...
        public void TriggerHurt()
        {
            ...
            // Stop looping sounds and play hurt sound
            StopLoopingSounds();
        ...
        public void TriggerDeath()
        {
            ...
            // Stop looping sounds and play death sound
            StopLoopingSounds();
            PlayOneTimeSoundEffect(dieSound);
